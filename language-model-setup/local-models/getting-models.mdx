...
```bash Terminal
interpreter --local --model <user>/<model>
```
...
```bash Terminal
interpreter --local --model Mistral-7B-Instruct-v0.1-GGUF
```
...
interpreter.local = True
interpreter.model = "Mistral-7B-Instruct-v0.1-GGUF"
interpreter.chat()
```
